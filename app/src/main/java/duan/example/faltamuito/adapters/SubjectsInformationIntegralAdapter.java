...
public class SubjectsInformationIntegralAdapter extends BaseAdapter {

	private List<Category> categoryList;
	private Context context;

	public SubjectsInformationIntegralAdapter(Context ctx, List<Category> categoryList){
		this.context = ctx;
	...
	public View getView(final int position, View view, ViewGroup parent) {
		...
		for(int i = 0; i < subjectList.size(); i++){
			if(subjectList.get(i).getHalf_integral().equals(categoryList.get(position).getName())){
				subjects ++;
